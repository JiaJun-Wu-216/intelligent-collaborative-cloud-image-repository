...
import type {SpaceCategoryAnalyzeResponse} from '@/api/EntityType.ts'
import {message} from 'ant-design-vue'
import {getSpaceCategoryAnalyze} from '@/api/SpaceAPI.ts'
...
const fetchData = async () => {
  loading.value = true
  const response = await getSpaceCategoryAnalyze({
...
const option = computed(() => {
  ...
  const sizeData = dataList.value.map((item) =>
    ((item.totalSize as number) / (1024 * 1024)).toFixed(2),
  ) // 转为 MB
